...
### 이더넷 허브
>  이더넷 네트워크에서 여러 대의 컴퓨터, 네트워크 장비를 연결하는 장치이다. 한대의 허브를 중심으로 여러대의 컴퓨터와 네트워크 장비가 마치 별 모양으로 서로 연결되며, 같은 허브에 연결된 컴퓨터와 네트워크 장비는 모두 상호 간에 통신할 수 있게 된다. 허브에 라우터나 Layer 3 스위치 등의 장비가 연결되어 있으면 이를 통해 더 높은 계층의 네트워크(WAN, MAN 등)과도 연결이 가능해진다.
...
> Router  
OSI 3(네트워크) 계층 장비로, Switch와 Bridge, Hub를 통해 LAN 구성 후 외부와 연결하기 위해 필요한 장비다. IP를 이용하여 한 네트워크에서 다른 네트워크로 패킷을 전달하기 위해 사용한다. 충돌 도메인과 브로드캐스트 도메인을 분리한다.

> 충돌(콜리전, Collision) 도메인 (개수 = Port 개수)  
Data Link Layer(=> Physical Layer의 장비는 충돌 도메인을 나눠주지 못함)에서 발생하는 현상이며, Switch나 Bridge가 충돌 도메인을 줄여주면서 충돌도 적어지게 만든다.  
충돌이 일어날 수 있는 영역을 말하며, **Switch나 Bridge**로 나뉠 수 있는 네트워크 그룹이다. Data Link Layer 장비부터 포트 하나당 충돌 도메인이 하나씩 발생한다.

> 브로드캐스트 도메인 (개수 = Network 개수)  
브로드캐스트가 발생하는 영역을 말하며, **라우터**로 나뉘는 네트워크 그룹이다. 라우터는 네트워크를 구분 지으며 브로드캐스트 메시지가 외부 네트워크에 송신되지 않도록 막는다.
